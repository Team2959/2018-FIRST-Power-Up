/******************************************************************************
*																			  *
*	PixyCam/PixyCam.cpp - Definition of class PixyCam.						  *
*																			  *
...
// Constructor
PixyCam::PixyCam(std::unique_ptr<SerialInterface> serialInterface) : _serialInterface{ std::move(serialInterface) } { }

// Set servo positions from [0,65535]
bool PixyCam::SetServos(unsigned s0, unsigned s1)
{
	return false;
}

// Set camera brightness from [0,255]
bool PixyCam::SetBrightness(unsigned brightness)
{
	return false;
}

// Set LED colors.  Each channel is in [0,255]
bool PixyCam::SetLED(unsigned red, unsigned green, unsigned blue)
{
	// Form our transmission buffer.
	// First byte is zero.
	// Second byte is 0xFD to identify this as a SetLED command.
	// Red/Green/Blue channels follow.
	char	buffer[5]{ 0x00, 0xFD, static_cast<char>(red), static_cast<char>(green), static_cast<char>(blue) };
	return _serialInterface->Send(buffer, sizeof(buffer)) == sizeof(buffer);
}
